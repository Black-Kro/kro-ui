<template>
  ...
  >
    <div class="kro-alert__icon">
      <kro-icon v-if="iconName" :icon="iconName" />
    </div>
    <div>
      <slot />
    </div>
    <div v-if="dismissible">
      <kro-button icon="close" @click="hide" />
    </div>
  </component>
</template>
...
export default {
  ...
  setup(props: any, { emit }: { emit: any }) {
    ...
    const iconName = computed(() => {
      ...
      if (type === 'info') return 'mdi:alert-circle'

      if (type === 'error') return 'mdi:alert'

      if (type === 'success') return 'mdi:check'
...
<style lang="scss">
.kro-alert {
  --kro-button-background: rgba(0, 0, 0, 0.12);
